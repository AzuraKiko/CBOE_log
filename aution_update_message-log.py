...
# ORDER_MESSAGE_TYPES = ['AuctionUpdateMessage','AddOrderMessage', 'OrderExecutedMessage', 'ModifyOrderMessage', 'DeleteOrderMessage']
ORDER_MESSAGE_TYPES = ['AuctionUpdateMessage']
...
def group_and_sort_logs(parsed_logs, reverse=False):
    # Map OrderID:Symbol
    order_id_symbol_map = {}
    ...
    for symbol, order_logs in grouped_logs.items():
        # for order_id, messages in order_logs.items():
            order_logs.sort(key=lambda x: x['timestamp'], reverse=reverse)
...
def main():
    ...
    grouped_sorted_order_logs = group_and_sort_logs(parsed_log, reverse=True)
